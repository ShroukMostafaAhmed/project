...
import { Plus, Search, Edit2, Trash2, Eye, RefreshCw, AlertCircle, EyeOff } from "lucide-react";
      ...
      {loading ? (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <table className="min-w-full table-fixed">
            <thead className="bg-slate-50">
              <tr>
                <th className="w-[18%] px-4 py-3 text-right text-xs font-semibold text-slate-500">الاسم</th>
                <th className="w-[15%] px-4 py-3 text-right text-xs font-semibold text-slate-500">الرقم القومي</th>
                <th className="w-[13%] px-4 py-3 text-right text-xs font-semibold text-slate-500">الهاتف</th>
                <th className="w-[18%] px-4 py-3 text-right text-xs font-semibold text-slate-500">البريد</th>
                <th className="w-[10%] px-4 py-3 text-right text-xs font-semibold text-slate-500">الحالة</th>
                <th className="w-[14%] px-4 py-3 text-right text-xs font-semibold text-slate-500 whitespace-nowrap">تاريخ التسجيل</th>
                <th className="w-[12%] px-4 py-3 text-right text-xs font-semibold text-slate-500">إجراءات</th>
              </tr>
            </thead>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-100 text-sm table-fixed">
            <thead className="bg-slate-50">
              <tr>
                <th className="w-[18%] px-4 py-3 text-right font-semibold text-slate-600 whitespace-nowrap">الاسم</th>
                <th className="w-[15%] px-4 py-3 text-right font-semibold text-slate-600 whitespace-nowrap">الرقم القومي</th>
                <th className="w-[13%] px-4 py-3 text-right font-semibold text-slate-600 whitespace-nowrap">الهاتف</th>
                <th className="w-[18%] px-4 py-3 text-right font-semibold text-slate-600 whitespace-nowrap">البريد</th>
                <th className="w-[10%] px-4 py-3 text-right font-semibold text-slate-600 whitespace-nowrap">الحالة</th>
                <th className="w-[14%] px-4 py-3 text-right font-semibold text-slate-600 whitespace-nowrap">تاريخ التسجيل</th>
                <th className="w-[12%] px-4 py-3 text-right font-semibold text-slate-600 whitespace-nowrap">إجراءات</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {filtered.length === 0 ? (
                ...
                filtered.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-600 truncate">{s.nationalId ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-600 truncate">{s.phone ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-600 truncate">{s.email ?? "—"}</td>
